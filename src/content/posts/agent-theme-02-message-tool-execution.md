...
> 模型说“请调用工具”只是一个意图。真正的 Agent 要把这句话变成可校验、可授权、可执行、可回填、可审计的一次副作用。

上一主题回答了控制平面如何推进循环。本篇沿着一条 tool call 往下走：消息怎样配对，工具怎样注册和调度，权限与沙箱怎样形成两道不同的门，文件修改怎样留下回滚凭据，检索工具又怎样把未知空间压缩成上下文。对应 Claude Code 源码系列 07–15，共 9 个独立 section。

![Agent 消息、工具与副作用链](/images/posts/agent-theme-02-message-tool-execution/agent-theme-02-message-tool-execution-handdrawn.png)

## 先建立一条统一的副作用链

三个系统都可以抽象成：模型输出 `tool_call` → runtime 找到契约 → 校验输入 → 决定是否允许 → 选择执行环境 → 运行工具 → 产生进度与最终结果 → 追加到消息/会话。差异集中在每一步由哪个层持有，以及边界是否默认存在。
