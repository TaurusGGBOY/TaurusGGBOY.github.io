...
上一节只说明了请求长什么样，还没有说明“谁来接这个请求”。Claude Code 支持四种 API provider：Anthropic 第一方 API、AWS Bedrock、Google Vertex AI 和 Microsoft Foundry。它们都尽量提供相似的 Messages API 外观，但认证、SDK、区域、模型名和部分请求字段都不同。

### 为什么必须先判断 provider

provider 不是请求里的一个普通字符串，而是 API 客户端工厂的路由结果。它至少决定四件事：
...
如果不先判断，Claude Code 就可能拿 AWS 凭证去构造第一方客户端，或者把第一方的 beta header 放进 Bedrock 不接受的位置。表面上都是 `messages.create()`，底层并不是同一条 HTTP 路径。
...
`APIProvider` 是封闭联合类型：`'firstParty' | 'bedrock' | 'vertex' | 'foundry'`。三个环境变量通过 `isEnvTruthy()` 解释布尔语义；源码支持布尔真值，以及忽略大小写和首尾空白后为 `1`、`true`、`yes`、`on` 的字符串。三个开关都没有生效时，回退到 `firstParty`。

这不是“挨个试连接，谁能连上就用谁”。优先级是 **Bedrock → Vertex → Foundry → firstParty**。源码也没有把多个开关同时打开视为配置错误；它只是返回第一个命中的 provider。因此这几个开关在正常配置中应该互斥。
...
这张表对应的是 `restored-src/src/services/api/client.ts::getAnthropicClient` 的分支。它先准备公共 headers、代理 fetch 和 timeout，再按 provider 动态加载 `AnthropicBedrock`、`AnthropicFoundry` 或 `AnthropicVertex`；没有第三方开关时，才构造标准 `Anthropic`。四个对象最后都被当作 Anthropic client 交给 `queryModel`，但源码明确承认第三方 client 只是被转换成统一类型，并不拥有第一方 client 的全部能力。

这里还藏着一个不能忽略的配置边界：`getAPIProvider()` 的优先级是 Bedrock、Vertex、Foundry，而 `getAnthropicClient()` 的环境分支顺序是 Bedrock、Foundry、Vertex。若多个开关同时为真，前者得到的 provider 标签甚至可能和后者真正构造的 client 不一致。源码没有为这种组合做冲突修正，所以不要把多开关当成“同时启用多个后端”。

### provider 会继续影响请求内容

选好 client 以后，主循环仍然复用同一个 `queryModel`，但请求参数会根据 provider 做局部调整。例如工具搜索的 beta header 在第一方/Foundry 与 Vertex/Bedrock 上使用不同值；对 Bedrock，header 还要放进 `extraBodyParams`，而不是普通的 `betas` 数组。Bedrock 的 inference profile、Vertex 的 region、Foundry 的 deployment ID，也都不能由第一方 API 的默认值替代。

请求关联 ID 同样不是无条件生成。`queryModel` 只有在 `getAPIProvider() === 'firstParty'` 且 `isFirstPartyAnthropicBaseUrl()` 返回真时，才生成 `clientRequestId` 并发送对应 header；第三方 provider 不走这条第一方日志关联路径。Analytics、账户信息、远程 settings 和模型能力缓存等其他模块也会重复读取 provider，决定是否启用各自的第一方逻辑。

最后要分清两个概念：`getAPIProvider()` 判断的是“配置选择了哪种接入模式”；`isFirstPartyAnthropicBaseUrl()` 判断的是 `ANTHROPIC_BASE_URL` 是否指向 Anthropic 官方 host。前者默认是 `firstParty`，后者在没有自定义 base URL 时也返回真，但它们不是同一个判断。下一篇的问题会专门追踪后一个判断。
