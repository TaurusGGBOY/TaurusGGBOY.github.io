...
## YNM-9527 的验证需要打开浏览器现场

核心任务的最后一段是：

> 完成后使用 LSP、Chrome 和相关测试验证。

IDE 可以先把当前文件和选区作为宿主上下文送进 Claude Code，模型再提出 Edit 或 LSP 调用；Chrome 则通过已经装配的外部协议提供页面状态、截图或操作结果。源码首先决定这些工具是否进入当前工具池，宿主现场随后才被翻译成 MCP/RPC 消息。

下面沿 YNM-9527 的“代码修复—浏览器复现—结果回流”路径，说明 IDE、Chrome 与 Claude Code 的边界。
