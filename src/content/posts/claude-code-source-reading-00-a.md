...
本文的核心贡献，是以公开的 Claude Code 源码快照为边界，沿可核验调用链重建生产级 Agent harness 的运行边界、状态流和工程取舍。它提供可复用的源码阅读坐标，但这些模块图和设计解释属于第三方重建，不是 Anthropic 的官方架构，也不能外推为普遍规律。
...
这篇解读沿三条线展开：先把断言放回证据层，再沿源码调用链重建机制，最后把哪些结论只能留在这个源码快照、哪些只能作为有限启示分开。
...
这套方法回答的是"系统怎样组织"和"这种组织意味着什么"，不是线上效果评测；功能启用率、故障率、时延与任务成功率没有进入研究设计。结构解释、效果结论和后文的数字与比较需要分开阅读。
...
这套分层就是源码阅读坐标：函数、类型和分支可以说明 2.1.88 实际怎么走；价值判断和未来方向必须保留限定条件。主张越离开调用链，越不能直接写成产品事实。
...
一条原则只有跨多个子系统反复出现，才具备架构解释力。单个函数使用 JSONL，只能说明它选择了某种格式；会话、sidechain、compact boundary 和读取时链修补都偏向追加式设计，才有理由把"可审计状态"上升为系统原则。因此，这 5 个价值和 13 条原则应读作作者基于公开材料和可观察源码提出的解释框架，而非 Anthropic 内部设计文档或一份同名的正式架构规范。
...
为了把这些问题落到执行路径上，可以用一句 `Fix the failing test in auth.test.ts` 作为阅读坐标：它可能先读测试文件，再搜索认证逻辑，运行一遍失败用例，修改代码，最后重新执行测试。每一次工具调用背后，系统都要处理本轮上下文、Bash 命令权限、工具结果回写、窗口压缩和 subagent 返回；请求依次经过上下文组装、模型调用、`tool_use`、权限检查、工具执行、结果回填、subagent 委派、压缩和 transcript 写入。这样，调用链成为主线，文件目录只是定位证据的坐标。

### 运行边界｜模型负责选择，harness 负责把选择变成动作
...
论文把这组边界与 approval fatigue 联系起来，并引用 Anthropic 的公开分析：用户批准了约 93% 的权限请求。高批准率使规则、分类器与 sandbox 的独立安全判断成为论文讨论的重点。纵深防御也有代价，多个层如果共享解析器、性能预算和上下文，它们的独立性可能低于设计图呈现的程度。论文 v2 引用了长复合命令退化成通用确认、sandbox parser differential 等外部安全分析，这些是特定版本窗口里的 Tier C 证据，适用范围限于相应版本；安全层数和失效模式的独立程度需要分别验证。
...
MCP、Plugin、Skill 和 Hook 都能扩展 Claude Code，但它们落在 Agent loop 的不同插入点，
...
从执行顺序看，subagent 先隔离上下文与执行边界，再获得并发能力；它同时缩小权限与工具面。代价是父 Agent 只拿到摘要，完整证据链不会自然回流；多个隔离 Agent 可能看不到彼此的局部发现，从而重复实现。局部决定都合理，合起来仍可能破坏全局一致性。
...
论文 v2 从系统范围、信任模型、Agent runtime、扩展架构、记忆与上下文、多 Agent 路由六个维度比较 Claude Code、OpenClaw 和 Hermes Agent。
...
把这些源码事实连成系统问题，论文的贡献可以压缩成三层。第一项，是把 Claude Code 的功能列表还原成一组可以讨论的设计选择：权限为什么需要多层边界，压缩为什么要逐级升级，Skill 与 Hook 为什么不能合并，resume 为什么恢复消息却不恢复临时权限。这些实现只有放到自治与控制、上下文成本、可审计状态等轴线上，才会显示出一致性。
...
在论文的框架里，这六类问题都指向会话级 harness 的边界：模型能力提高以后，permission gate 不会自动变成外部审计系统；上下文窗口变长以后，跨月项目不会自动获得可靠记忆；subagent 数量增加以后，局部验证不会自动形成全局一致性；代码生成更快以后，开发者也不会自动理解 Agent 改过的系统。
