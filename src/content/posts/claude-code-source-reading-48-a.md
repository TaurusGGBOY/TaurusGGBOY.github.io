...
- **诊断顺序与建设顺序相反**：故障排查先问「哪一层失守」，能力建设却要从内向外补——先写清楚 prompt 和 CLAUDE.md，再配 hook 与权限，最后才设计循环与图。
...
先给结论：**prompt 只是五层嵌套体系的最内层。把 Claude Code 说成「一个大 prompt」，等于把五层边界压缩成一层**——它解释不了「为什么模型想调用工具却被权限拒绝」（Harness），解释不了「为什么上下文塞满后模型开始忘事」（Context），也解释不了「为什么同一个任务可以被 `/loop` 挂起来反复执行直到收敛」（Loop）。五层的正确关系是嵌套，不是并列：
...
五层框架的用途不是背诵，而是诊断：输出格式乱先查 Prompt，幻觉先查 Context，危险操作未拦截先查 Harness，过早停或无限循环先查 Loop，串行慢与结果漂移先查 Graph。框架比 2.1.88 的源码快照更稳定——版本升级后文件会移动，但「输入在哪一层被改写、权限在哪一层被拒绝、结果在哪一层没有抵达受众」的责任边界不变。
