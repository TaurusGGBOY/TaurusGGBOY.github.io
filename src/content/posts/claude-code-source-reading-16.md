...
> 先读取 CLAUDE.md、事故单和相关代码；按照项目约定修复，不要改变项目之外的文件。

模型请求发出前，Claude Code 会把 system prompt、项目层级的 CLAUDE.md、会话历史、当前事故 prompt、git 状态和动态工具结果分开准备，再在调用前合并。于是模型同时知道“应该遵守什么”和“YNM-9527 目前查到了什么”，但这两类内容并不是同一个来源。

下面从这次请求的三条上下文通道开始，追踪项目规则如何进入最终 API 参数。
