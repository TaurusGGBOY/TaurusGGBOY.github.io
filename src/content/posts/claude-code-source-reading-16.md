...
如果面试题追问“Prompt 模板和 To-Do List 为什么让模型更聚焦”，可以先给出可迁移的层次：**系统约束 → 当前任务 → 结构化状态/任务清单 → 近期消息 → 长期记忆 → 工具结果**。本篇源码能确认 system、message 和 tool schema 的协议分工，以及 `CLAUDE.md` 按需进入 message context；任务清单作为结构化状态的设计建议，则是基于该分层的工程抽象，不应伪装成 2.1.88 有一个统一的 Todo 状态机。

同样，首次生成、follow-up、槽位补全等业务路由也不能从 prompt 组装函数直接推出。源码确认的是上下文怎样装配；具体产品是否保存 `conversation_id`、任务槽位和 schema 版本，要看业务层实现。
