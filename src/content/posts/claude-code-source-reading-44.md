...
Buddy 的执行边界先明确：它不选择工具，也不进入 `queryLoop()`；它只在普通 REPL 外叠加持久身份、回合反馈和终端表现三层产品能力。这样陪伴感不会变成另一条执行链。
...
本文全部引用 `@anthropic-ai/claude-code@2.1.88` 的 `restored-src/` 还原源码。代码块只保留证明控制流所需的字段；每个代码块后标注证据位置。缺失的 observer/command 实现不做推断。
...
| 阶段 | 关键符号 | 位置 | 证据状态 |
| --- | --- | --- | --- |
| 类型 | `CompanionBones` / `CompanionSoul` / `StoredCompanion` | `src/buddy/types.ts` | 已确认 |
| 身份 | `companionUserId()` / `getCompanion()` / `roll()`（`userId + SALT`） | `src/buddy/companion.ts` | 已确认 |
| 命令注册 | `feature('BUDDY')` 动态加载 `/buddy` | `src/commands.ts` | 已确认（命令实现 ⚠️ MISSING） |
| 发现 | `useBuddyNotification()` / `buddy-teaser` / 15s | `src/buddy/useBuddyNotification.tsx` | 已确认 |
| 注入 | `getCompanionIntroAttachment()`（同名去重） | `src/buddy/prompt.ts` | 已确认 |
| 回合后 | `fireCompanionObserver()`（query 事件流结束后 `void` 调用） | `src/screens/REPL.tsx` | 已确认（observer 实现 ⚠️ MISSING） |
| 状态 | `companionReaction` / `companionPetAt` | `src/state/AppStateStore.ts` | 已确认 |
| 动画 | `TICK_MS 500` / `BUBBLE_SHOW 20` / `FADE_WINDOW 6` / `PET_BURST_MS 2500` | `src/buddy/CompanionSprite.tsx` | 已确认 |
| 布局 | `companionReservedColumns()`（100 列门槛、12 列 sprite、36 列气泡） | `src/buddy/CompanionSprite.tsx` | 已确认 |
...
1. **验证身份的确定性。** 在自己的配置里找到 `userID`（或 `oauthAccount.accountUuid`），用还原源码的 `companionUserId()` 优先级与 `roll()` 逻辑推演你的 Buddy 的物种与稀有度；重复启动 Claude Code 观察结果是否一致。再尝试直接修改配置里的 `rarity` 字段，验证 `getCompanion()` 的 `{ ...stored, ...bones }` 覆盖行为。
...
1. Buddy 与主 Agent 唯一的提示词联系是什么？什么条件下不注入？
2. `companionReaction` 与 `companionPetAt` 为什么放进 AppState 而不是 GlobalConfig？
3. 本章最重要的证据缺口是什么？

<details>
<summary>参考答案</summary>

1. **`companion_intro` attachment。** 它告诉 Claude 输入框旁边坐着独立 watcher；当用户直接叫 Buddy 名字时主 Agent 少说一句。功能关闭、companion 缺失、`companionMuted` 为真、或历史中已有同名 `companion_intro` 时返回空数组不注入（`getCompanionIntroAttachment()`）。Tool、`canUseTool` 和 permission context 都保持原值。

2. **生命周期不同。** 名字、性格、孵化时间需要跨进程保留，放进 `GlobalConfig.companion`；气泡和心形动画只是眼前一段 UI 的生命周期（10 秒 timeout、2500 毫秒心形窗口），放进 AppState 让 observer 不必重写整个 companion、动画 tick 也不持续写磁盘。

3. **`commands/buddy/index.js` 与 `buddy/observer.ts` 的实现不在还原文件中。** 能确认命令注册、持久化字段、attachment、回合后触发和动画消费端；soul 生成逻辑、完整子命令、observer 的提示词/模型/失败策略都是黑盒，不能由 UI 结果反推。
