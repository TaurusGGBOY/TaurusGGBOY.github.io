...
- MCP 不是"读配置然后列工具"，而是一条带生命周期的管线：**config → transport → initialize → capability 门控发现 → 本地 Tool/Resource 包装 → 两层权限 → 统一 `tool_result`**；任意一段失败，server 都可能停在 `pending`、`failed` 或 `needs-auth`，不会把半成品能力交给模型。
...
**先区分两种"冲突"。** 第一种是**共享工作区冲突**：几个 teammate 没有隔离 worktree，直接在同一个 cwd 写文件，未提交修改互相可见。这不是 Git merge 失败，而是执行阶段已经发生了覆盖风险。第二种才是**分支合并冲突**：每个 teammate 在独立 worktree 完成了 commit，lead 把这些分支按顺序合回主线时，Git 发现双方修改了同一片内容，或者一边修改、一边删除了同一个路径。
