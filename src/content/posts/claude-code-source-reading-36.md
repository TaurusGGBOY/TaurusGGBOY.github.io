...
| 机制 | 关键符号 | 位置 | 证据状态 |
| --- | --- | --- | --- |
| 模型来源 | `getUserSpecifiedModelSetting()` / `getMainLoopModel()` | `src/utils/model/model.ts` | 已确认 |
| 别名解析 | `parseUserSpecifiedModel()` 五类别名 + `[1m]` | `src/utils/model/model.ts` | 已确认 |
| 模型表 | `getBuiltinModelStrings(provider)` / `getModelStrings()` | `src/utils/model/model.ts` | 已确认 |
| Provider 路由 | `getAPIProvider()` 环境优先级 | `src/utils/model/providers.ts` | 已确认 |
...
1. `getUserSpecifiedModelSetting()` 返回 `null` 与返回 `undefined` 有什么区别？
2. 为什么 `validateModel()` 对已知别名直接返回 valid？
3. `CLAUDE_CODE_SKIP_BEDROCK_AUTH` 开启后，请求就一定成功吗？
4. `FallbackTriggeredError` 本身会发起第二次请求吗？
...
| --- | --- | --- | --- |
| `saveGlobalConfig(...)` | `getGlobalClaudeFile()`，通常是 `~/.claude.json` | `verbose`、自动 compact、主题、编辑器模式、通知 | 不会；这是全局应用状态文件 |
| `updateSettingsForSource('userSettings', ...)` | 通常是 `~/.claude/settings.json`；cowork 模式可能是 `cowork_settings.json` | 权限默认模式、`alwaysThinkingEnabled`、fast mode、语言、自动更新通道 | 会，写用户层 settings |
