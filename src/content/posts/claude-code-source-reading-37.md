...
## YNM-9527 从办公室 CLI 续到远端设备

用户在另一台设备的 Remote Control 中输入：

> 继续处理 YNM-9527。先恢复当前 worktree、后台测试、teammate 状态和待决权限，再完成验证；不要重新执行已经成功的副作用。

本地执行端仍持有工作区、工具和 Query Loop；控制端只发送输入并接收事件。中间的 Bridge 或会话服务负责认证、连接关系和消息转发，不因此获得本地文件的直接执行权。

下面沿“本地启动—远程接管—结果回流”这条路径，说明服务器在整个远程流程中的位置。
