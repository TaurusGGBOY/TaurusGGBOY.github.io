...
午间发布窗口临近，支付组想把同一张工单交给三种宿主验证：你在终端里交互式排查，CI 用 `claude -p` 做一次只读复现，内部诊断脚本再通过 SDK 保留会话，远程同事则从 Remote Control 接管你已经打开的目录。

四种入口提交的都是同一段任务：

> 请检查支付服务中的金额单位问题：订单使用优惠券后，结算页显示为 99.90 元，支付回调却记录为 9991 分；请查清原因、修复并运行测试。

交互式入口需要 TUI，`claude -p` 需要结构化输出，SDK 由宿主消费事件，Remote 则把输入转发给远端 session；它们的外壳不同，但可能共享 Query、工具和权限内核。真正要问的不是“命令名字是否相同”，而是谁接收输入、谁处理权限、Agent 循环在哪个进程里继续。

下面沿着这条“同任务、多入口”的现场，看 `main` 怎样识别宿主，再选择适配层。
...
- [Claude Code 权限模式比较](https://claudcod.com/blog/claude-code-permissions/)
