...
这次不改事故内容，只改变发送方式。用户可以在交互式 CLI、claude -p、SDK 或 Remote Control 中提交同一段任务：

> 请检查项目中的 YNM-9527：订单使用优惠券后，结算页显示为 99.90 元，支付回调却记录为 9991 分；请查清原因、修复并运行测试。

交互式入口需要 TUI，claude -p 需要结构化输出，SDK 由宿主消费事件，Remote 则把输入转发给远端 session；它们的外壳不同，但可能共享 Query、工具和权限内核。

下面沿着这条“同任务、多入口”的路径，看 main 怎样识别宿主，再选择适配层。
