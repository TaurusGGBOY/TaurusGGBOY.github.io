...
- `claude` 命令的六种入口，交互式 REPL、`claude -p`、Agent SDK、MCP server、Bridge、direct-connect，共用**同一套内核**，区别在于宿主组合。判别点只有两个，输入与权限从哪里来，Agent 循环在哪里运行。
...
`QueryEngine` 负责处理会话，也是下一篇的主角。这里先看它怎样被使用，
