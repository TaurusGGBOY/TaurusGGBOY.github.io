...
整个系列仍然只讨论 `@anthropic-ai/claude-code@2.1.88`。源码能够直接确认的内容写成事实，调用关系连接出的职责写成解释，缺少运行证据的部分明确标成推断。从第 01 章开始，文章会固定观察一张具体的支付工单：同一件事如何经过模型、工具、权限、状态和宿主，最终变成一次可恢复、可扩展的 Agent 执行。
