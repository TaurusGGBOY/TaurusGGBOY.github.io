...
这三个概念把时间线和后面的源码阅读接在一起：先问发布物暴露了什么，再问源码能证明什么，最后才讨论外部分析推导出的风险。

2026 年 3 月 31 日，用户只执行了一次正常的 `npm install`，拿到的却不只是编译后的 CLI：`cli.js.map` 还带着可以逐条还原的 TypeScript 文本。不到两个小时，源码地址被公开，随后出现撤包、GitHub 镜像清理和借“泄露版 Claude Code”传播窃密木马的攻击。

这篇导读要回答的不是“泄露了多少行代码”，而是一个发布工件怎样把构建、分发、复制和社会工程串成事故链。可确认的起点在 npm 包：原本只服务调试的 source map 把源码文本带进了公开发布物；现有材料没有支持 Anthropic 内网被入侵的证据。

## 先让 YNM-9527 跑起来

为了让后面的章节始终围绕同一条执行线，我们固定一个事故场景。用户在 CLI 中输入：

> 请检查项目中的 YNM-9527：订单使用优惠券后，结算页显示 99.90 元，支付回调却记录为 9991 分；请查清原因、修复并运行测试。

Claude Code 会先读取项目和运行环境，再决定是否需要查资料、调用工具、询问权限或继续追问。之后每一章只截取这次任务的一小段：有时是命令入口，有时是工具调用，有时是压缩后的远程恢复。

这条事故线就是本系列的阅读地图。下面先回到 source map 公开发布的起点，再看我们为什么能够沿着同一个任务逐步走进 Claude Code 的内部流程。
