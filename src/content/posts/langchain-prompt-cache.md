...
updated: 2026-09-10
description: "从 LangChain 的 lookup 调用链走到模型服务端的 KV cache，解释结果缓存、前缀匹配，以及 Anthropic、OpenAI 和 DeepSeek 的缓存控制差异，并给出命中验证方法。"
...
### 换成 DeepSeek，这些设置还有效吗

DeepSeek 官方 API 默认开启服务端上下文缓存，LangChain 正常发送请求就能参与缓存复用。缓存前缀由服务端自动识别和持久化，手动控制参数要按兼容接口分别看：

| DeepSeek 接口 | 参数 | 官方支持情况 |
| --- | --- | --- |
| Anthropic 兼容接口 | 工具和消息内容块上的 `cache_control` | 忽略，无法借此指定缓存断点或 TTL |
| Responses 兼容接口 | `prompt_cache_key`、`prompt_cache_retention` | 不支持，参数会被静默忽略 |

这意味着请求成功返回，只能证明接口接受了请求。把前面的 Anthropic 缓存中间件接到 DeepSeek 上，或沿用 OpenAI 的 cache key，都不能据此判断缓存控制已经生效。[Anthropic 兼容表](https://api-docs.deepseek.com/guides/anthropic_api/)和 [Responses 兼容表](https://api-docs.deepseek.com/guides/responses_api/)明确列出了这些参数的行为。

回到手册问答，DeepSeek 当前的命中规则还有一个关键条件：公共前缀须先保存为完整的缓存单元。按官方示例，可能出现下面的过程：

```text
第一次：P + 问题 A → 保存完整请求前缀
第二次：P + 问题 B → P 尚未独立缓存；系统识别并持久化公共前缀 P
第三次：P + 问题 C → 可以匹配已保存的 P
```

因此，第二次换问题没有命中，可能是公共前缀还在形成缓存单元。请求边界、跨请求的公共前缀检测，以及长序列的固定间隔都会产生缓存单元；实际命中还受构建时间和条目可用性影响。[DeepSeek 上下文缓存说明](https://api-docs.deepseek.com/guides/kv_cache/)将其定义为尽力而为的机制。

验证时保持稳定内容在前、问题在后，连续发送几次请求，并检查 Chat Completions 原始 `usage` 中的 `prompt_cache_hit_tokens` 与 `prompt_cache_miss_tokens`。LangChain 的 `InMemoryCache` 仍可独立使用；排查服务端缓存时设置 `cache=False`，让请求实际到达 DeepSeek。
...
关闭 LangChain 结果缓存，固定模型、工具和其他配置。支持显式断点的接口先标记稳定前缀；DeepSeek 使用自动缓存。然后按顺序执行：
...
对 DeepSeek，在 C 之后、D 之前再增加一次只改问题的请求，给服务端识别和持久化公共前缀的机会，再比较缓存读取量。

如果 B 能命中、C 不能，优先看手册末尾有没有可查找断点；对 DeepSeek，则检查公共前缀是否已经形成独立缓存单元。如果 B 也不能，再查该服务的长度条件、缓存有效期、最终请求内容，以及模型和适用的分组设置。先把顺序请求跑通，再增加并发，可以减少多个冷请求同时到达带来的干扰。
...
落到文档问答服务，先做三个记录就够了：每次实际出站的请求数、供应商返回的缓存 token 明细、首 token 和总响应耗时。再把问题放在手册之后，按供应商能力设置断点或等待公共前缀自动持久化，运行 A 到 D。你就能定位，这次优化省掉的是一整次模型调用，还是一段前缀的重复计算。
