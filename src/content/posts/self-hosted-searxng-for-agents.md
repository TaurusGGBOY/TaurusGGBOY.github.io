...
SearXNG 要成为可供 Agent 调用的本地搜索服务，验收对象不只是首页：进程要能常驻，HTTP 与 format=json 要能返回，常用引擎要逐一知道成功或失败的原因，Agent 还要有固定入口和运行约定。

本文以我的部署为案例，整理从安装、配置、服务管理到逐引擎验收和 Agent 接入的路径。端口、操作系统、代理和上游状态都属于部署条件；Docker、Linux 原生安装和 macOS 原生安装的命令不能直接互换。
