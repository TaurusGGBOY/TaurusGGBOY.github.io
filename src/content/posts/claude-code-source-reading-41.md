...
## YNM-9527 的结论怎样变成团队记忆

用户在 Team 完成验收后输入：

> 把金额单位错误的根因、最终方案和回归测试命令写入团队记忆，让下一位处理支付问题的人能找到它。

Claude Code 先按项目作用域定位 memdir，再通过索引和主题文件区分 private 与 team 内容；Team Memory 还要经过 pull、watch、push 和路径/密钥扫描，不能把本地文件直接当成所有成员都能看到的共享事实。

下面从这条收尾输入开始，追踪目录、索引、相关记忆和团队同步如何共同限制注入范围。
