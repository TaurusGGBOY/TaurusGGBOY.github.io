...
### 面经回看｜企业 Agent 的安全边界不能只靠模型

面试中问“怎么防越权”时，本系列可以给出 Claude Code 2.1.88 的具体答案：模型提出命令不代表进程获准执行；命令先解析，权限规则再裁决，沙箱限制进程可接触的资源，启动后的超时、取消和输出收口继续保护宿主。权限与沙箱是正交层，任一层的失败都不能被另一个层的“已允许”替代。

这是对本仓库实现的解释，不是所有企业 Agent 都具备同等隔离强度的证明。实际系统还需要根据租户、凭据、网络和副作用类型补充策略，不能把本地源码的安全边界直接外推成通用安全结论。
