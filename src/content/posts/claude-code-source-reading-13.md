...
### 企业 Agent 的安全边界不能只靠模型

把问题放到企业 Agent 的安全边界上，Claude Code 2.1.88 给出了一条具体链路：模型提出命令不代表进程获准执行；命令先解析，权限规则再裁决，沙箱限制进程可接触的资源，启动后的超时、取消和输出收口继续保护宿主。权限与沙箱是正交层，任一层的失败都不能被另一个层的“已允许”替代。
