...
> 创建独立 worktree，把金额计算、回调解析和前端复现分别交给合适的 sub-agent 或 teammate；集成测试放到后台。

Claude Code 会创建 Task，先把它注册到 AppState，再让前台继续接收 YNM-9527 的其他消息；大段测试输出写入文件，查询状态或停止任务时从统一入口取回。完成后，终态通知把结果重新送回模型或用户。
