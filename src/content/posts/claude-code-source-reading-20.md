...
### 首次请求与 follow-up 不能由 resume 代答

“首次生成和多轮补充路由”需要单独建模，不能由 resume 代答。本篇源码直接证明的是 JSONL transcript、`parentUuid`、resume、continue 和 fork 如何重建会话分支；它不能推出一个完整的业务意图状态机，也不能凭 `/resume` 证明系统会自动区分改参数、回答澄清问题或开启新任务。那些判断应由业务层保存的任务状态、槽位和版本信息完成。
