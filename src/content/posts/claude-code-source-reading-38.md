...
本系列能直接回答的，是如何用 session、request、`tool_use_id`、task、token、cost、重试和终态把一次执行串起来；这些字段可以帮助定位 Bad Case 落在模型请求、工具、权限、网络还是恢复层。源码没有提供任务成功率黄金集、幻觉率、RAG 质量评分、专家抽检流程，也没有决定“哪个 Agent 应该做 SFT”；这些属于需要另建数据集、标注和回归门禁的评测工程。

因此面试回答要把两层分开：先用本章的 trace 和诊断信号找到故障位置，再用固定样本、人工盲审和修复后的回归集判断质量是否真的改善。前一层是源码事实，后一层是工程方案。
