...
工具调用不准确时，不能只修改 Prompt。模型的 `tool_use` 只是一份候选动作，不能直接执行：注册表先按名称解析，工具契约提供 JSON Schema 和描述，运行时还要用 `validateInput`、权限规则与工具自身的前置条件再次收口；失败则生成带原 `tool_use_id` 的错误 `tool_result`，交给下一轮模型决定修正、换工具或结束。

这条链能证明结构化校验和错误回填存在，不能证明模型一定会自动修正，也不能把一次成功调用说成 exactly-once。幂等、重试上限和副作用去重仍要看具体工具及其业务实现。
