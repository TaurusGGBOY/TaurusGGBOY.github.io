...
用户看到的是一个开关，服务端收到的却是一组按 provider、模型和运行模式计算出的 `betas` header。开关只解除发送限制，不能单独决定某个能力是否进入请求；每项 Beta 还要通过自己的模型与运行条件。本篇沿这条合并路径追踪条件如何进入请求。
...
### 面经回看｜Function Calling 的关键不是“让模型会调用”

遇到“工具调用不准确怎么办”时，最小而可靠的回答是：模型的 `tool_use` 只是一份候选动作，不能直接执行。注册表先按名称解析，工具契约提供 JSON Schema 和描述，运行时还要用 `validateInput`、权限规则与工具自身的前置条件再次收口；失败则生成带原 `tool_use_id` 的错误 `tool_result`，交给下一轮模型决定修正、换工具或结束。

这条链能证明“结构化校验和错误回填存在”，不能证明模型一定会自动修正，也不能把一次成功调用说成 exactly-once。幂等、重试上限和副作用去重仍要看具体工具及其业务实现。
...
内置工具大多通过 `buildTool` 构造。它把缺省值集中到一个地方，
