...
模型可能先提出 Read、Grep 或 WebSearch 的 tool_use；运行时从当前注册表找到对应对象，读取名称、Schema、权限元数据和 call 实现，校验通过后才真正执行。结果回到模型，用户才会看到“找到了金额单位问题”这类说明。

上一章解释事件怎样流动，本章把模型看到的工具契约与宿主持有的执行对象接起来。
