...
答案先放在前面，**2.1.88 并没有把“Skill 能否用 `/` 调用”硬编码成 Plugin 专属能力。**没有组织策略时，项目里的 `.claude/skills/<name>/SKILL.md`、用户目录下的 `skills/<name>/SKILL.md`，以及兼容的 `.claude/commands/<name>.md` 都可以生成可调用的 prompt command。你看到“必须安装 Plugin”，通常是因为管理员打开了 `strictPluginOnlyCustomization`，而不是因为 Slash 命令解析器要求 Plugin。
...
5. 工具可用性检查未通过，`ENABLE_LSP_TOOL` 未开启、插件未 enabled（配置只来自已启用插件）、文件扩展名没有匹配 server、spawn/initialize 失败、或 server 还没完成握手。此时 LSP 没有通过工具可用性检查，扩展是条件性装配，不涉及模型是否愿意调用。
...
结果由 `strictPluginOnlyCustomization` 影响，它来自 managed 的 `policySettings`，
