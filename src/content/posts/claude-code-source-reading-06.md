...
这里的 `terminal` 不是“shell terminal”，而是循环的“收工信号”：`query()` 通过 `yield*` 接住 `queryLoop()` 的返回值后再 `return` 出去。它是一个对象，不是进程，也不是屏幕终端。静态源码里可以看到它至少包含统一字段 `reason`（表示本轮为什么结束），某些分支还会附带 `turnCount`（`max_turns`）或 `error`（`model_error`）等字段。这样上层能根据终止原因区分：正常完成、达到 `max_turns`、恢复失败、模型异常、用户中断等。
...
但同一源码里没有 `tasks/LocalWorkflowTask/*` 和 `commands/workflows/*` 实现文件，静态证据不足以复原完整编排算法（例如子任务拆分策略、是否有 run 级统一预算）。
...
- `maxBudgetUsd` 仍是 `QueryEngine` 层面的单次 `submitMessage` 截断。若 dynamic workflow 需要 run 级总预算，通常来自更外层调度/编排实现，而不是这里 `queryLoop()` 本身可见逻辑。

所以看 dynamic workflow 的“总预算表现”时建议先按三层检查：先看子代理是否有 `maxTurns / taskBudget`，再看 QueryEngine 是否单会话设了 `maxBudgetUsd`，最后再看动态工作流运行器有没有 run 级预算和并发控流。本文源码只能直接验证前两层。
