...
把 Agent 是什么、一次请求怎么走，以及为什么不能只靠一个大 Prompt，压缩成一条源码可核对的链：**模型输出消息，消息中的 `tool_use` 触发执行器，执行结果以配对的 `tool_result` 回到消息历史，`queryLoop()` 再决定是否继续请求模型**。权限、预算、取消、Stop hook 和错误恢复不是模型的“自觉”，而是循环外侧的控制边界。

这也解释了 Agent 与固定 Workflow 的差别：本章源码直接证明的是运行时会根据上一轮真实结果继续或退出；“ReAct”“Planner/Executor/Critic”“Plan-and-Execute”是对这类控制流的设计归纳，不是 2.1.88 中名为这些角色的统一实现。它们可以用来比较不同方案，但不能当作源码事实。
