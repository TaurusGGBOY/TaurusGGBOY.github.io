...
这张图只画循环真正依赖的状态：当前消息、工具结果、恢复信号和预算。它们共同决定下一步是继续请求模型，还是从某个终止出口返回。

## 同一个事故为什么会连续调用工具

你在工单里给出的要求很短，但执行现场不会只发生一次模型调用：先读优惠券结算函数，再查支付回调的字段定义，接着核对最近一次提交，最后运行回归测试。每次工具结果回来，模型都要重新判断“证据够不够”“下一步是否需要副作用”“现在能不能结束”。

模型先要求读取代码，收到结果后继续要求搜索或编辑，再用 Bash 跑测试；如果根因已经确认，也可能直接给出结论。取消、工具错误、预算耗尽和正常完成从不同出口离开循环。

上一章说明谁保存 turn，本章说明它为什么会一轮一轮继续：把这张金额单位工单看成一台有两个主要出口的 Agent 状态机。

## 先把 Agent loop 看成一台两出口机器

先不引入“规划”“反思”等抽象词，只看一次迭代怎样从输入走到两个出口。
