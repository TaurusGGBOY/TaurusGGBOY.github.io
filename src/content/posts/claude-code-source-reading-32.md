...
## YNM-9527 在终端里是怎样“动起来”的

用户在交互式 CLI 输入核心任务，并在过程中执行：

> /keybindings

输入框、权限确认、工具进度和模型流都由 REPL 送进 React/Ink 状态；终端 resize 或 Ctrl-C 又会产生新的控制事件。Ink 根据下一帧的组件树差异重绘，用户看到的是同一个 YNM-9527 任务不断更新，而不是整屏重复打印。

下面沿输入、流式响应、弹窗和取消的顺序，说明根组件、PromptInput、消息列表与渲染循环怎样接起来。
