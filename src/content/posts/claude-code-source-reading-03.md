---
title: "Claude Code 源码解读 03：第一次提问前，它到底做了什么"
...
09:17，你在支付服务目录里运行 `claude`，还没有输入金额单位工单。终端先短暂停在启动画面：当前目录是谁、这份仓库是否可信、账号有没有有效 token、项目 settings 是否覆盖了用户配置、哪些 MCP 和 Plugin 能够进入工具池，这些问题都要先有答案。

几秒后你输入“请检查支付服务中的金额单位问题，先给证据和计划”。模型还没有看到这句话，Claude Code 已经解析命令行、确定 cwd 和 session、读取 settings、检查认证与目录信任，并装配工具和 MCP。屏幕上看似还在“启动”，其实是在给后面的查询准备所有依赖。

上一章的时间线从请求开始，本章把时间线往前推：值班工程师按下 Enter 以前，main 怎样把一个工作目录和一组配置变成可执行的运行环境。
...
| TTY、TUI 与 REPL | TTY 表示进程连接着可交互终端；TUI 是在终端中绘制的界面；REPL 原意是 Read-Eval-Print Loop，即“读取、执行、输出、继续等待”的交互循环。 | Claude Code 持续接收 prompt、流式展示回答、弹出权限确认，并在多轮对话之间保留状态。 |
| React 与 Ink | React 用组件、状态和 effect 描述会变化的界面；Ink 作为终端 renderer，把组件树画成终端文本。 | 消息列表、输入框、Spinner、工具进度和权限弹窗会同时变化。声明式组件比手工计算光标位置、擦除旧文本和重画终端更容易维护。 |
...
传统编程语言 REPL 读入一行表达式，求值后打印结果。Claude Code 沿用了“循环等待下一次输入”这个外形，并把 `Eval` 扩展为一整段 Agent 执行：组装上下文、请求模型、等待 `tool_use`、确认权限、执行工具，再把结果交还模型。因此，REPL 在本文中指持续运行的交互宿主，覆盖输入、渲染、权限和会话状态。
...
我们先看最外层入口 `restored-src/src/entrypoints/cli.tsx`。它先检查参数，为特殊命令保留 fast path，再按需导入完整程序。
...
- `startCapturingEarlyInput()`：零参数函数，在完整 CLI 动态加载前开始缓存终端输入，避免模块加载期间的按键丢失。
- `cliMain()`：零参数的普通启动入口；仅当前面的 fast path 都未结束进程时才会被加载和等待。

这里有两个值得注意的细节。

第一，`--version` 输出版本后立即返回，完整 CLI 保持未加载状态。第二，普通路径使用动态 `import()`，特殊分支全部排除后才加载 `main.tsx`。进程启动早于完整 Claude Code 初始化。

`remote-control` 更能说明这个区别。它会先检查 OAuth、功能开关、最低版本和组织策略，然后直接进入 `bridgeMain()`。这个分支不需要先挂载本地 REPL。因此，我们不能拿交互式启动顺序去解释所有运行模式。

## main 先给这次运行贴上标签

普通路径进入 `restored-src/src/main.tsx` 后，`main()` 先判断当前是否为非交互模式，并写入进程级运行状态；模型请求仍在后续阶段。
...
- `hasSdkUrl`：布尔值；任一参数以 `--sdk-url` 开头就为 `true`。
...
- `clientType`：由前面的入口和参数解析得到；这段初始化代码只把已经确定的运行时值写入进程状态，静态片段无法列出完整候选集合。
...
这里还出现了 `eagerLoadSettings()`。它会提前解析 `--settings` 和 `--setting-sources`，确保后续模块从同一配置来源创建对象。启动顺序在这里直接维护状态一致性。
...
- `allowDangerouslySkipPermissions`：布尔值；`true` 表示调用方请求危险跳过权限路径，`false` 保持常规权限路径。该请求仍要通过 `setup()` 的环境与安全条件检查，检查失败会终止启动。
- `worktreeEnabled`：布尔值；`true` 时 `setup()` 可能创建或切换 worktree，因此这里不提前调用 `getCommands()`；`false` 时命令加载可以并行开始。
- `worktreeName`：`string | undefined`。字符串由 CLI 或上游配置提供；省略时由后续 worktree 逻辑决定名称。
- `tmuxEnabled`：布尔值，表示是否启用与 worktree 相关的 tmux 准备。
- `sessionId`：开放字符串。传入时先经过 `validateUuid()`；省略时跳过自定义会话切换。校验失败后的处理由验证函数和上游参数流程决定。
- `commandsPromise`：启用 worktree 时为 `null`，使命令加载等待最终 cwd；否则立即保存 `getCommands(preSetupCwd)` 返回的 Promise，与 `setup()` 并行。

这段代码采用条件并行：普通目录中，命令和 Agent 定义可以与 `setup()` 并行加载；worktree 模式则先由 `setup()` 切换工作目录，再从切换后的项目读取命令和 Agent。

我们再看 `restored-src/src/setup.ts` 的入口。它接收 cwd、权限模式、worktree 和会话 ID，准备的是本次运行所在项目的具体环境。
...
- `allowDangerouslySkipPermissions`：必填布尔值，调用方必须显式提供。`true` 和 `bypassPermissions` 都会触发额外安全检查；`false` 保持常规权限流程。
- `worktreeEnabled`：必填布尔值；`true` 允许本次初始化准备 worktree，`false` 保持普通项目目录路径。
- `worktreeName`：必填位置上的 `string | undefined`。传入字符串表示显式名称；省略时进入后续命名分支。它是开放输入，源码未声明固定候选列表。
- `tmuxEnabled`：必填布尔值；控制是否为相应 worktree 路径准备 tmux 会话。
- `customSessionId`：可选的 `string | null`。只有非空字符串才进入 `switchSession()`；`null`、省略和空字符串都会保留当前 bootstrap session。

完整源码在 `customSessionId` 后还有 `worktreePRNumber?: number` 与 `messagingSocketPath?: string`，节选为聚焦主线而省略。前者来自 PR/worktree 上下文，后者是开放的 socket 路径；两者省略时均为 `undefined`，后续初始化逻辑分别跳过 PR 关联和显式消息 socket。

`setup()` 做的事情很多，但可以归纳为两个目的。

一个是建立运行基础，例如 Node.js 版本、工作目录、worktree、会话 ID、日志与后台服务。另一个是尽早拒绝不安全组合。例如 `bypassPermissions` 或 `--dangerously-skip-permissions` 会同时触发 root/sudo、容器、沙箱和网络条件检查。权限模式既是请求阶段的工具执行规则，也是启动阶段能否继续运行的前置条件。
...
先要澄清名字：这个选项只跳过 `showSetupScreens()` 里的 `Onboarding` 组件。上一节的 `setup()` 以及 Node.js 版本、工作目录、权限安全检查、目录信任、MCP 审批和自定义 API Key 确认仍按各自条件执行。
...
- `devChannels`：可选 `ChannelEntry[]`，省略时为 `undefined`。
...
- 返回值：`Promise<boolean>`；`true` 表示本次展示过 Onboarding，`false` 表示该界面被条件分支跳过。这个值只描述界面展示状态，整个 setup 的成功或失败由外层控制流决定。

用户正常完成引导时，`completeOnboarding()` 会把标志位写入全局配置，使后续进程也能读取：
...
- `current`：`saveGlobalConfig()` 传入的当前 `GlobalConfig`；展开它可以避免覆盖 API、项目、主题等无关配置。
- `hasCompletedOnboarding: true`：完成函数只写入真值；首次引导的跳过条件会读取这个持久化标志。
- `lastOnboardingVersion`：写入当前构建的 `MACRO.VERSION`。这个版本字符串供后续构建判断是否需要重新展示引导。
...
默认全局文件是 `~/.claude.json`。源码还兼容 Claude 配置目录下旧的 `.config.json`；如果设置了 `CLAUDE_CONFIG_DIR` 或使用带 OAuth 后缀的配置，实际文件名也会变化，所以修改前应以 `getGlobalClaudeFile()` 的路径规则和本机现有文件为准。
...
很多初始化工作都需要读配置，而不同配置要到各自的校验阶段完成后才能进入可信状态。

交互模式会先创建 Ink root，再调用 `showSetupScreens()`。首次运行、登录、目录信任等阻塞式界面都在这一阶段处理。只有用户确认信任当前目录，REPL 才会真正挂载。
...
**功能：** 这个 React effect 在 REPL 挂载后触发插件启动检查，远程会话则直接跳过。effect 只启动一次异步检查；组件卸载时不会由这段代码取消后台安装。
...
- `void performStartupChecks(...)`：明确丢弃 Promise 返回值，让检查在 effect 中异步推进；函数内部负责捕获错误，React effect 继续完成本次调用。
...
- `setAppState`：必填的 `SetAppState` 状态更新函数；后台安装始终通过它更新插件相关的应用状态。
...
- 返回值：`Promise<void>`，只表示异步流程已经结束，不携带“安装了几个插件”之类的结果。
...
- `sourceJsonlFile`：`loadConversationForResume()` 的第二个参数，类型是 `string | undefined`；这里传 `undefined`，让加载器按会话元数据定位 JSONL。
- `result`：恢复加载结果，源码类型允许为对象或 `null`。本文省略了上游未找到会话时的处理分支。
...
- `transcriptPath`：可选字符串；来自 `result.fullPath`，让跨目录恢复能够找到原始 transcript。`result.fullPath` 省略时，恢复逻辑沿用当前会话的默认路径。
- `resumeContext`：必填上下文对象，包含当前 cwd、初始 AppState、Agent 定义以及可选的协调模式 API；恢复逻辑用这组对象重建当前运行环境。
- `processedResume`：处理后的恢复结果，包含可继续交给宿主的消息、会话和 Agent 状态；旧进程中的临时对象由当前运行环境重新创建。
...
恢复的本质是读取持久化证据，再构造本轮需要的状态；旧网络连接、计时器、React 组件实例和临时闭包都由新进程重新创建。
...
- `replProps`：必填 `REPLProps` 对象，承载 REPL 所需的命令、工具、会话状态和回调；具体字段由 `screens/REPL.tsx` 的 `Props` 类型约束，属于结构化运行时输入。
- `renderAndRun`：必填回调，签名是 `(root: Root, element: React.ReactNode) => Promise<void>`。它决定怎样把 React 节点交给 Ink 并等待终端应用退出。
- `App` 与 `REPL`：由交互式挂载路径动态导入，加载时机晚于前面的模式分流。
...
Bridge 也遵守类似的边界。`initReplBridge()` 会检查功能开关、OAuth、组织策略和版本条件；任一条件失败都会返回 `null`，本地 REPL 随后继续运行。远程控制是挂在会话上的可选能力，本地请求路径独立成立。
...
Claude Code 的启动过程先分流运行模式，再逐步建立可信的项目环境。`setup()` 固定 cwd、worktree、权限和基础服务；会话逻辑创建或恢复可持久化状态；信任确认约束项目插件何时能够生效；REPL、Bridge 和无头模式最后接入各自的输入输出通道。

源码同时表明，有些工作必须阻塞启动，有些工作可以并行，有些工作会在首屏之后继续。判断一个初始化步骤属于哪一类，关键不在它叫不叫 `setup`，而在后续代码是否必须依赖它的结果。

本文讨论的版本边界仍是 `@anthropic-ai/claude-code@2.1.88` 的 source map 还原源码。
