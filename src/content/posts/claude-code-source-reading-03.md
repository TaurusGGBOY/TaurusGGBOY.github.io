...
> 请检查项目中的 YNM-9527：订单使用优惠券后，结算页显示为 99.90 元，支付回调却记录为 9991 分；请查清原因、修复并运行测试。

模型还没有看到这句话，Claude Code 已经解析命令行、确定 cwd 和 session、读取 settings、检查认证与目录信任，并装配工具和 MCP。屏幕上看似还在“启动”，其实是在给后面的查询准备所有依赖。

上一章的时间线从请求开始，本章把时间线往前推：先看 main 如何把一句事故任务变成可执行的运行环境。
