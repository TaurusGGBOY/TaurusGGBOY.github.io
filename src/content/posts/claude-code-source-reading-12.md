...
于是模型即使提出了 Edit、Bash 或 MCP 调用，也不会直接越过副作用边界。Schema 和业务校验先处理输入，Hook 可能拒绝或改写，permission mode 为 `ask` 时再把未决请求交给宿主；你在终端确认后，执行器才拿着最终输入继续。

上一章说明了一次调用有哪些门，本章只聚焦其中最容易改变路线的一道：当修复金额转换的 Edit 已经准备好时，权限引擎怎样在 `allow`、`ask`、`deny` 和 `updatedInput` 之间作出决定。
...
- [Claude Code image analysis](https://claudcod.com/blog/claude-code-image-analysis/)
