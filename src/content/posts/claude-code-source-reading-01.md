...
终端只显示一条请求，源码里却已经出现多块区域：Host 接收输入，Query Core 组织回合，Context/Model 决定下一步，Execution/State 把决定落到工作区，Extensions 负责 MCP、Skill 和外部宿主。

上一章的 source map 让我们知道“能看见什么”；从这里开始，要看同一个事故是怎样穿过这些区域的。下面先建立全局地图。
