...
- Claude Code 不是一堆互相调用的文件，而是一个围绕**会话内核（Query Core）**组织的 Agent 系统：外围区域不断向内核提供输入、上下文、执行能力和扩展能力，内核把这一切拼成下一轮请求。
...
区域归属：**Execution & State（Permission 子区域）**。权限决定与副作用执行必须在同一条工具执行路径上连续完成，中间任何"提前放行"都会拆开这条链。地图告诉我们：权限不是独立的审批弹窗，而是执行路径上的强制关卡——回到上面 `toolExecution.ts` 的代码，只有允许分支才进入 `tool.call()`，这个不变量一旦被绕过，就是 Bug 3。
...
如果做成单体，Bug 1 到 Bug 3 仍然会出现，但很难被命名：缓存顺序问题会混在"渲染逻辑"里，压缩重试会混在"上下文处理"里，权限绕过会混在"命令解析"里。四区划分真正的价值不是消灭 bug，而是让每一次失败都能被放进一个区域、问出正确的问题。
...
三题做完，回到正文开头那句话：判断标准不是函数名，而是它维护的状态和它回到 Query Core 的边界。
