...
最后收一下安全边界。第一，`-p` 会跳过 workspace trust 对话框。CLI 帮助文本直接提醒只能在信任目录使用，主入口也会在 print 分支应用完整环境变量。自动化系统必须自己决定 cwd、项目配置和环境变量是否可信。
...
第三，stdout 在 `stream-json` 下是机器协议，诊断必须去 stderr。宿主也应按 schema 的未知事件分支处理扩展类型；2.1.88 的消息联合已经包含 rate limit、task、hook、auth、compact boundary 等系统事件。集成代码仍应固定并记录 Claude Code/SDK 版本，测试权限超时、断流、重复 response 与部分结果。
...
| 机制 | 关键符号 | 位置 | 证据状态 |
| --- | --- | --- | --- |
| SDK 类型 | `query()` 双重载（string / AsyncIterable） | `src/entrypoints/agentSdkTypes.ts` | 已确认 |
| 入口判定 | `isNonInteractive`（-p/--init-only/--sdk-url/isTTY） | `src/main.tsx` | 已确认 |
| 无头状态 | `createStore(headlessInitialState, onChangeAppState)` | `src/main.tsx` | 已确认 |
| 输入归一化 | `getInputPrompt()` / `getStructuredIO()` 包装 user message | `src/cli/print.ts` | 已确认 |
| 主循环 | `runHeadlessStreaming()` → `ask()` | `src/cli/print.ts` | 已确认 |
| 输出格式 | `text / json / stream-json` switch | `src/cli/print.ts` | 已确认 |
| stdout 保护 | `installStreamJsonStdoutGuard()` | `src/utils/streamJsonStdoutGuard.ts` | 已确认 |
| 权限宿主 1 | `hasPermissionsToUseTool()` + 非 allow 生成 error tool_result | `src/cli/print.ts`, `src/services/tools/toolExecution.ts` | 已确认 |
| 权限宿主 2 | `createCanUseTool()` + Hook/SDK `Promise.race` | `src/cli/structuredIO.ts` | 已确认 |
...
3. **对比 `-p` 与 SDK 的权限行为。** 在 `-p` 模式下执行一个未预先 allow 的写操作，观察模型收到的是 `is_error: true` 的 `tool_result`；再用 SDK 的 `canUseTool` 回调实现同样的审批，观察 `control_request` / `control_response` 的往返。

## 自测

1. 为什么 `stream-json` 需要替换 `process.stdout.write`？
2. `-p` 模式下工具权限返回 `ask`，会发生什么？
3. `--output-format=json` 与 `--json-schema` 是同一层吗？
4. SDK 的权限请求与 PermissionRequest Hook 是什么关系？
