...
需要把上一版结论限定为官方 `/buddy` 路径：普通用户不修改身份 seed、不修改 Claude Code 时，确实做不到 100% 抽到 `legendary`；但“控制 seed 后也做不到”是不准确的。社区已经实现了可重复命中指定稀有度的工具；它利用的是确定性映射，而不是让 1% 概率在重复尝试中变成保底。
