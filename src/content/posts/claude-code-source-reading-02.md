...
两者都能组织“模型调用工具、工具返回结果、模型继续推理”的循环，但运行边界不同：LangGraph 提供通用编排框架，Claude Code 则把编排层与模型、工具、权限、上下文和终端交互一起做成产品运行时。
...
这里的关键问题是，既然骨架只有 30 行，为什么 `queryLoop()` 要写 1488 行？
...
答案先放在前面，用 LangGraph 开发编程 Agent 是拿通用编排框架搭自己的系统，Claude Code 连编排层本身都是自研的。
