...
上午 09:18，你已经在 `checkout-service` 仓库里确认过当前分支没有本地改动。你把刚收到的金额单位工单交给 Claude Code：

> 请检查支付服务中的金额单位问题：订单使用优惠券后，结算页显示 99.90 元，支付回调却记录为 9991 分；请查清原因、修复并运行测试。

终端先出现项目规则和文件读取，随后是金额字段搜索、模型提出的 `tool_use`、权限询问和测试输出。你看到的是一条不断刷新的工作流，源码里却要把它拆成输入、上下文、模型流、工具结果和下一轮推理。最后那句“根因在回调适配层”只是尾声，前面已经发生了多次消息回填。

上一章把这张工单放进了系统地图，本章把现场改成时间线：从你按下 Enter 开始，模型怎样得到上下文，工具怎样把结果送回来，Agent 又怎样决定是否继续。
