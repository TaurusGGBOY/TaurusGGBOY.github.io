...
Claude Code 可能先读 CLAUDE.md 和失败测试，再搜索金额字段；模型返回 tool_use，工具结果回来后才决定是否编辑代码、运行测试或继续搜索。最后的回答只是这条链的尾声，前面已经发生了多轮消息和结果回填。

上一章画的是“有哪些区域”，本章把它们按时间排列：从输入进入、模型流到达、工具执行，再回到下一轮请求。
