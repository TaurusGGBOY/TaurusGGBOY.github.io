...
先说答案：**插件不是另一套工具协议，而是一种能力的分发和生命周期容器。**
...
这条链最重要的边界是：**发现不等于信任，安装不等于激活，插件已启用也不等于其中每个组件都能成功运行。** Marketplace 来源策略、项目信任、manifest Schema、相对路径、组织策略、组件自己的配置校验，都会在不同阶段拒绝或降级插件。
...
1. **Marketplace 是目录。** 它告诉 Claude Code 有哪些插件、每个插件的来源、版本、依赖和可选组件，不是插件进程本身。
2. **Plugin 是分发单元。** 它是一组文件和配置，可以包含 `.claude-plugin/plugin.json`，也可以依靠 `commands/`、`agents/`、`skills/`、`hooks/hooks.json` 等约定目录。
3. **Active components 是运行时能力。** 命令进入命令列表，Agent 进入 Agent 定义，Hook 进入生命周期回调，MCP/LSP 配置则交给各自的连接或 server manager。它们不会因为同处一个插件目录就共享同一种执行模型。
...
注意图中的 `reload`。它不是重新下载的同义词，而是清缓存、重读已启用插件，并把命令、Agent、Hook、MCP 和 LSP 状态切换到当前会话。也正因为安装和激活被拆开，交互式会话可以避免在首屏阶段被一次 Git clone 卡住。

## 安装的第一步不是复制文件，而是确定作用域
...
`PluginScopeSchema()` 接受四个精确值。`user` 写入用户级设置；`project` 写入项目共享设置；`local` 写入当前项目的个人覆盖；`managed` 表示企业或系统管理的只读安装。这里没有 `undefined` 或 `null` 这一档作用域。

不过，“Schema 支持 managed”不等于“用户可以安装到 managed”。`scopeToSettingSource()` 在 `restored-src/src/utils/plugins/pluginIdentifier.ts` 中明确拒绝这种写入：
...
`scope` 是前面的四值联合类型；返回值只可能是 `userSettings`、`projectSettings` 或 `localSettings` 对应的可编辑来源。传入 `managed` 会抛错，而不是悄悄回退到 `user`。这一区分让“管理员声明的策略”和“用户主动安装的意图”不会写进同一配置层。
...
这里可以得到一个很实用的结论：安装不是“把根插件复制一下”。它同时更新声明层和物化层，并且处理依赖闭包。反过来，写入 `enabledPlugins` 成功也不代表组件已经出现在当前命令列表中——安装成功消息本身就提示用户运行 `/reload-plugins` 才能激活。

## 版本化缓存为什么是生命周期的一部分

插件不是直接从 marketplace 目录执行。`cacheAndRegisterPlugin()` 会计算版本，把内容移动到版本化路径，再记录 `installPath`。路径生成在 `restored-src/src/utils/plugins/pluginLoader.ts`：
...
版本目录把“插件名字”和“本次实际安装的内容”分开。更新时可以写入新版本并把旧版本标为 orphan；后台清理不是立刻递归删除，而是在 `.orphaned_at` 标记超过七天后再移除。这样做既支持版本切换，也避免一次失败更新直接抹掉仍可能需要排查的旧内容。
...
`manifestPath` 是绝对或已解析的 manifest 路径；`pluginName` 来自 marketplace 条目或 fallback name；`source` 是插件来源标识。文件缺失会返回默认对象，文件存在但 JSON 损坏或 Schema 不通过则抛错，两者不是同一种情况。也就是说，约定目录足以构成一个简单插件，而坏 manifest 不会被当成“没有 manifest”绕过。

Manifest 中的组件路径也不是任意路径。命令支持单个相对路径、相对路径数组，或“命令名到 metadata”的映射；Agent 文件要求相对 Markdown 路径；Skill 指向相对目录；Hook 可以内联，也可以引用相对 JSON；MCP/LSP 可以引用配置文件或内联对象。路径 Schema 要求以 `./` 开头，独立校验命令还会在 Schema 前检查 `commands`、`agents`、`skills` 中的 path traversal。

这解释了 manifest 的真正作用：它不是授权书，而是一份**受约束的组件索引**。它能说“插件希望加载这个文件”，不能说“这个文件因此拥有更高权限”。

## 从插件目录到六种运行时组件

`createPluginFromPath()` 位于 `restored-src/src/utils/plugins/pluginLoader.ts`。它先读 manifest，再自动探测没有被 manifest 显式覆盖的标准目录：
...
`pluginPath` 是已解析插件根目录；`manifest.commands` 等字段为 `undefined` 时才自动探测约定目录，显式配置存在时则以配置路径为准。四个结果都是布尔值，没有 `null` 状态，并行探测只优化 I/O，不改变后续装配顺序。
...
这段摘自 `addPluginScopeToLspServers()`；MCP 的 `addPluginScopeToServers()` 同样使用 `plugin:${pluginName}:${name}`，但额外保存 `pluginSource`。`pluginName` 和 `name` 都是开放字符串，来源分别是已加载插件和 server 配置；`scope` 固定为 `dynamic`，不是安装阶段的 `user/project/local/managed`。前者表示 server 是运行时动态来源，后者表示插件启用意图存在哪一层，不能混为一谈。

命名空间解决的是碰撞，不是授权。两个插件都声明 `typescript` server 时，它们不会在 key 上互相覆盖；至于 server 的可执行命令、环境变量、启动是否成功，还要经过 MCP/LSP 自己的 Schema 和生命周期。
...
`clearAllCaches()` 同时清插件、命令、Agent、Skill prompt 和 output style 等 memoization；`clearPluginCacheExclusions()` 让显式 reload 重新扫描 orphan exclusion。`loadAllPlugins()` 是允许物化新内容的 full load；后面的命令和 Agent 加载可并行，因为 full load 已先完成并预热 cache-only 结果。`enabled`、`disabled`、`errors` 始终是数组，不用 `null` 表示空集合。
...
`setAppState` 接收基于旧 `AppState` 的更新函数；`needsRefresh` 在成功消费刷新后固定为 `false`。`pluginReconnectKey` 每次加一，用变化信号让 MCP connection effect 重跑。`reinitializeLspServerManager()` 即使当前没有 LSP 插件也会调用，这样移除最后一个 LSP 插件时不会留下旧配置。`loadPluginHooks()` 内部采用 clear-then-register，旧 Hook 会保留到新集合准备好，而不是在清缓存的一瞬间全部消失。
...
这个函数只接收 `setAppState`，没有可选 flags；信任检查为假就提前返回。后续后台安装即使抛错也会被捕获并记录，不阻断 Claude Code 主启动。它保护的是“项目未信任时不要因为项目声明而安装插件”，不是宣称已信任项目中的每段代码都安全。

第二道是 marketplace 来源策略。加载器读取 `strictKnownMarketplaces` 和 `blockedMarketplaces`。allowlist 即便是空数组也表示“拒绝全部”；blocklist 只有非空才构成有效限制。策略存在但 marketplace 来源无法解析时，源码明确 fail-closed，把插件记为 `marketplace-blocked-by-policy`，而不是绕过检查继续加载。

第三道是插件级组织策略。根插件和依赖闭包都会经过 `isPluginBlockedByPolicy()`，避免一个允许的插件把被禁依赖带进来。

第四道是文件与 Schema。损坏的 JSON、错误的字段类型、路径穿越、丢失的组件文件会产生不同错误。部分组件缺失可以让插件带着错误继续加载；manifest 损坏、来源策略失败或依赖不完整则可能让插件进入 disabled 或直接返回 `null`。这不是统一的“忽略所有错误”，而是按失败是否破坏身份、来源和执行边界来降级。
...
更新不是在原目录上原地覆盖。新内容进入新的 versioned cache，安装记录指向新 `installPath`，旧版本进入 orphan 清理流程。Marketplace 的 `autoUpdate` 也有明确回退：配置显式给出布尔值时按该值执行；字段为 `undefined` 时，部分官方 marketplace 默认开启，其他来源默认关闭。`null` 不在 Schema 允许范围内。

禁用同样不只是把菜单开关变灰。设置层需要改变 `enabledPlugins`，缓存层可以暂时保留已安装版本，active layer 则必须清理命令/Agent 缓存、阻止移除插件的 Hook 继续触发、让 MCP 重连，并让 LSP manager 丢弃旧 server 配置。保留缓存和撤销运行能力是两件事：前者便于恢复与版本管理，后者决定当前会话的安全边界。
...
命令、Skill、Hook、Agent、MCP 和 LSP 虽然能够一起分发，却不会一起执行。它们分别进入命令、Agent、Hook、MCP 和 LSP 子系统，继承各自的配置、命名空间、故障与权限边界。插件只是把这些能力组织起来，并没有抹平它们之间的差异。

最需要记住的仍是三个“不等于”：Marketplace 可见不等于可信，安装成功不等于已经激活，插件启用不等于所有组件都获得许可。源码中的项目 trust、组织 policy、Schema、路径校验、依赖 demotion 和组件级权限，正是为了让扩展能力不会变成一个无条件执行入口。
