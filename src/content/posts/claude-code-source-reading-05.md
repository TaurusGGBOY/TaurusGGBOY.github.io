...
你把金额单位工单从终端转给一个内部诊断脚本。脚本先提交“只读检查支付回调”，得到金额字段和调用链；十几秒后，业务同事又补充了一条消息：“把刚才确认的根因写进修复计划，但先不要改文件。”这两次输入必须属于同一段会话，否则第二次请求会失去第一轮的文件读取结果和权限决定。

QueryEngine 收到后会保存消息历史、工具集合、权限上下文、文件状态和取消信号。第一轮找出金额单位后，第二轮仍能沿用刚才的文件、`tool_result` 和权限决定，而不是重新开始一次陌生请求。

上一章只比较入口，本章继续往里推：同一张工单的连续 turn 究竟由谁持有，SDK 宿主又怎样消费这段状态。
